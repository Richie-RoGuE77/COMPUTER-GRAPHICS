/*
program to draw a line using Bresenham’s Line Algorithm (BLA) for lines with slopes positive and less than 1.
*/
...
main()
{
	...
	while(run)
	{
		printf("\n\nBresenham Line Algorithm\n\tFor Slope Positive and Less than 1\n");
		...
		{
			//if dy is +ve and dx -ve -> exchange points and recaluculate dx dy
			if(( dy > dx )&& (dy > 0 && dx < 0) ) {
				temp = a;
				a = b;
				b = temp;
			...
			outtextxy(a.x-40,a.y-15,"(x1,y1)");
			outtextxy(a.x+20,a.y-5,") <- +ve slope");
			outtextxy(a.x+30,a.y+10,"and slope < 1");
			outtextxy(b.x+5,b.y+5,"(x2,y2)");
			getch();
			closegraph();
		}
		else{
			printf("Slope is Not Positive and Not Less than 1\n\a");
		}
